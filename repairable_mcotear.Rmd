...
The first thing we want to know when analyzing data from repairable systems is whether there is a **trend** in the times between failure. In other words, we want to know if the times between failure are getting longer, shorter, or staying constant. We can inspect this first using graphs. There are a few different types of plots that can help with this inspection such as the Duane Plot, Total Time on Test Plot, and the 

### Duane Plot

In the package I did not specifically include a funtion to generate a Duane Plot, but given the functions I created it is easy enough to creae a Duane Plot. A Duane Plot is a plot of the **cumulative failure rate** ($\frac{N(t_{i})}{t_{i}}$) versus the time of failure ($t_{i}$), where $t_{i}$ represents the time of the $i$th failure, and $N(t_{i})$ represents the number of failures that have occurred up to time $t_{i}$. (Some implementations of the Duane Plot suggest inverting the value of the y-axis such that $\frac{t_{i}}{N(t_{i})}$, the cumulative mean time between failure, is plotting instead of the cumulative failure rate.) The values plotted on a Duane Plot are plotted on a log-log scale. When points fall along a straight line this gives an indication that a **power-law process** is a good is a good model for the data. If the points follow an increasing or decreasing slope, or follow a constant zero-slope line this gives an indication of whether the failure rate is increasing or decreasing (this depends on whether you plot $\frac{N(t_{i})}{t_{i}}$ or $\frac{t_{i}}{N(t_{i})}$), or staying constant.  
...
```{r}
rocof_identical <- rocof(t = split(amsaa$Time, amsaa$System), by = NULL)
...
head(amsaaa_identical)
# Merge our data set with the df_rocof by the Time variable
```

Now I can create the Duane Plot as follows. In the following plot uses the "rocof" ($\frac{N(t_{i})}{t_{i}}$) on the y-axis. Since the ROCOF is decreasing this indicates that the system(s) are improving over time.  

```{r, fig.height=4, fig.width=4}
ggplot(amsaaa_identical,
  aes(
    x = Time,
    y = rocof)) +
  scale_x_log10() +
...
Alternatively I could plot the inverse of "rocof", the "mtbf" on the y-axis. The following plot uses the "mtbf" ($\frac{t_{i}}{N(t_{i})}$) on the y-axis. Since the MTBF is increasing this indicates that the system(s) are improving over time.  
...
amsaa3 <- merge(amsaa, df_rocof2, by = c("Time", "System"))
head(amsaa3)
